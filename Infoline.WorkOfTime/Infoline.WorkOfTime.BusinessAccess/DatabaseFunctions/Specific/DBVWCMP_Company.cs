...
namespace Infoline.WorkOfTime.BusinessAccess
{
    partial class WorkOfTimeDatabase
    {
        ...
        public VWCMP_Company GetVWCMP_CompanyByNameOrCode(string name, string code,string taxNumber, DbTransaction tran = null)
        {
            using (var db = GetDB(tran))
            {
                return db.Table<VWCMP_Company>().Where(a => a.name == name || a.code == code||a.taxNumber==taxNumber).Execute().FirstOrDefault();
            }
        }
