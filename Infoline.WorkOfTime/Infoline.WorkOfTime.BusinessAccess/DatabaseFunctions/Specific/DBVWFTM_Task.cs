...
namespace Infoline.WorkOfTime.BusinessAccess
{
	...
	partial class WorkOfTimeDatabase
	{
		...
		public VWFTM_Task[] GetVWFTM_TaskByDueDateYear(int year, Guid? customerId, Guid? planId, DbTransaction tran = null)
		{
			using (var db = GetDB(tran))
			{
				var query = "SELECT id,customer_Title,customerId,taskTemplateId,lastOperationStatus,lastOperationDate,dueDate,type_Title,taskPlanId_Title FROM VWFTM_Task WITH (NOLOCK) WHERE YEAR(dueDate) = " + year;
		...
		public VWFTM_Task[] GetVWFTM_TaskByAssignUserIdNotNullAndAssignableUsersNew(DateTime date, List<Guid?> userIds, DbTransaction tran = null)
		{
			...
			using (var db = GetDB(tran))
			{
				...
				if (userIds != null)
				{
					return db.ExecuteReader<VWFTM_Task>("SELECT  * FROM VWFTM_Task WHERE assignableUserIds IN (" + string.Format("'{0}'", string.Join("','", userIds)) + ") or  assignUserId IN (" + string.Format("'{0}'", string.Join("','", userIds)) + ") and planStartDate >= '" + string.Format("{0:yyyy-MM-dd HH:mm}", startdate) + "' and dueDate <= '" + string.Format("{0:yyyy-MM-dd HH:mm}", enddate) + "' AND lastOperationStatus >= " + (int)EnumFTM_TaskOperationStatus.GorevBaslandi + "AND lastOperationStatus <" + (int)EnumFTM_TaskOperationStatus.GorevDurduruldu + "").ToArray();
				}
