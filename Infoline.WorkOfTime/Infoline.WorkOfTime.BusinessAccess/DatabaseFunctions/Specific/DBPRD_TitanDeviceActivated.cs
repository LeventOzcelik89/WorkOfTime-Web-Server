...
namespace Infoline.WorkOfTime.BusinessAccess
{
    partial class WorkOfTimeDatabase
    {
        ...
        public PRD_TitanDeviceActivated GetPRD_TitanDeviceActivatedByImei(string imei, DbTransaction tran = null)
        {
            using (var db = GetDB(tran))
            {
                return db.Table<PRD_TitanDeviceActivated>().Where(a => a.IMEI1 == imei).Execute().FirstOrDefault();
            }
        }
