...
namespace Infoline.WorkOfTime.BusinessAccess.Models
{
    ...
    public class ItemDayEnd
    {
        public string _mainTitle { get; set; }
        public string _content { get; set; }

        public ItemDayEnd(string mainTitle, string content)
        {
    ...
    public class EndOfDayMailService
    {
        public List<ItemDayEnd> ItemsEnd { get; set; } = new List<Models.ItemDayEnd>();
        ...
        public List<VWCMP_Company> INV_TodayInsertCustomer { get; set; }
        ...
        public EndOfDayMailService(WorkOfTimeDatabase db, string _url)
        {

            //Proje Yönetimi
            PRJ_TodayInsertProject = db.GetVWPRJ_ProjectByInsertToday(0).ToList();
            PRJ_TodayInsertProjectKMs = db.GetVWPRJ_ProjectTimeLineByTodayInsert(0).ToList();
            ...
            //Bugünkü Etkinlikler
            INV_CompanyMeetingsTodayInsert = db.GetVWINV_CompanyMeetingByTodayInsert(0).ToList();
            INV_CompanyNotificationsTodayInsert = db.GetVWINV_CompanyPersonNotificationByTodayInsert(0).ToList();
            INV_CompanyPresentationsTodayInsert = db.GetVWINV_CompanyPersonPresentationByTodayInsert(0).ToList();
            //Bugünkü Etkinlikler
            ...
            INV_TodayRequestPermitOffer = db.GetVWINV_PermitRequestOfferByToday(0).ToList();
            ...
            FTM_TodayOpenTask = db.GetVWFTM_OpenTodayTask(0).ToList();
            FTM_TodaySolutionReportTaskOperation = db.GetFTM_TodayReportSolution(0).ToList();
            FTM_TodaySolutionAcceptTaskOperation = db.GetFTM_TodayAcceptSolutionTaskOperation(0).ToList();
            ...
            //Satışlar
            CRM_TodayInsertPresentation = db.GetVWCRM_PresentationInsertByToday(0).ToList();
            INV_TodayInsertCustomer = db.GetVWCMP_CompanyInsertCustomerByToday(0).ToList();
            CRM_TodayHappeningMeeting = db.GetVWCRM_ContactTodayHappeningMeeting(0).ToList(); // Toplantı İsmi ile ilgili bilgi yok.
            CRM_TodayCancelMeeting = db.GetVWCRM_ContactTodayCanceldMeeting(0).ToList(); // Toplantı İsmi ile ilgili bilgi yok.
            var CRM_MostMeetingPerformingPerson = db.GetVWCRM_ContactUserByMostMeetingPerformingPerson().GroupBy(a => a.User_Title).Select(a => new
            {
                //Data = a.ToArray(),
                Title = a.Key,
                Count = a.Count()
            }).OrderByDescending(a => a.Count);
            CRM_PresentationTodayPositiveResults = db.GetVWCRM_PresentationPositiveResultByToday(0).ToList();
            //Satışlar

            if (PRJ_TodayInsertProject.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Proje Yönetimi",
                    "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/PRJ/VWPRJ_Project/Index>Bugün eklenen proje sayısı</a>" + "*" + PRJ_TodayInsertProject.Count()
                        ));
            }
            ...
            if (PRJ_TodayInsertProjectKMs.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Proje Yönetimi",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/PRJ/VWPRJ_Project/Index>Bugün eklenen proje kilometre taşı sayısı</a>" + "*" + PRJ_TodayInsertProjectKMs.Count()
                        ));
            }
            ...
            if (PRJ_TodayEndProjects.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Proje Yönetimi",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/PRJ/VWPRJ_Project/Index>Bugün biten proje adedi</a>" + "*" + PRJ_TodayEndProjects.Count()
                        ));
            }
            ...
            if (PRJ_TodayStartProjectKMs.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Proje Yönetimi",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/PRJ/VWPRJ_Project/Index>Bugün başlayan kilometre taşı sayısı</a>" + "*" + PRJ_TodayStartProjectKMs.Count()
                        ));
            }
            ...
            if (PRJ_TodayEndProjectKMs.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Proje Yönetimi",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/PRJ/VWPRJ_Project/Index>Bugün biten proje kilometre taşı adedi</a>" + "*" + PRJ_TodayEndProjectKMs.Count()
                        ));
            }
            ...
            if (INV_CompanyMeetingsTodayInsert.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Bugünkü Etkinlikler",
                    "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/Account/Index>Bugün eklenen toplantı sayısı</a>" + "*" + INV_CompanyMeetingsTodayInsert.Count()
                        ));
            }
            ...
            if (INV_CompanyNotificationsTodayInsert.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Bugünkü Etkinlikler",
                    "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/Account/Index>Bugün eklenen mevcut duyuru adedi</a>" + "*" + INV_CompanyNotificationsTodayInsert.Count()
                        ));
            }
            ...
            if (INV_CompanyPresentationsTodayInsert.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Bugünkü Etkinlikler",
                    "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/Account/Index>Bugün gerçekleşecek sunum sayısı</a>" + "*" + INV_CompanyPresentationsTodayInsert.Count()
                        ));
            }
            ...
            if (INV_TodayPermitAcceptPersons.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "İnsan Kaynakları",
                    "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/INV/VWINV_Permit/Index>Bugün izinli olan personel sayısı</a>" + "*" + INV_TodayPermitAcceptPersons.Count()
                        ));
            }
            ...
            if (INV_TodayInsertCustomer.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Satış Faaliyetleri",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/CRM/VWCRM_Presentation/Index>Yeni eklenen müşteri sayısı</a>" + "*" + INV_TodayInsertCustomer.Count()
                        ));
            }

            if (CRM_MostMeetingPerformingPerson.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Satış Faaliyetleri",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/CRM/VWCRM_Presentation/Index>En çok toplantı gerçekleştiren personel sayısı</a>" + "*" + CRM_MostMeetingPerformingPerson.Count()
                        ));
            }
            ...
            if (CRM_PresentationTodayPositiveResults.Count() > 0)
            {
                ItemsEnd.Add(new Models.ItemDayEnd(
                    "Satış Faaliyetleri",
                     "<a style ='font-size:12px;font-weight:bold;color: #000; text-decoration: none;' href=" + _url + "/CRM/VWCRM_Presentation/Index> Bugün eklenen potansiyel sayısı</a>" + "*" + CRM_PresentationTodayPositiveResults.Count()
                ));
            }
        }
