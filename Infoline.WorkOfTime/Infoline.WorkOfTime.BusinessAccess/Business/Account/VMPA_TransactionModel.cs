		...
		public void UpdateDataControl(VWPA_TransactionConfirmation[] confirmations, string statusDescription, Guid userId)
		{
			...
			for (int i = 0; i < confirmations.Count(); i++)
			{
				...
				if (confirmations[i].confirmationUserIds == null && !control)
				{
					if (this.direction == 2)
					{
						confirmations[i].status = (Int16)EnumPA_TransactionConfirmationStatus.Red;
						...
						if (confirmations[i].createdby.HasValue)
						{
							...
							var getDeclineUser = db.GetVWSH_UserById(userId);
							var text = "<h3>Sayın " + user.FullName + ",</h3>";
					...
					else if (this.direction != 3)
					{
						...
						{
							if (confirmations[i].transactionId.HasValue)
							{
								var transaction = db.GetPA_TransactionById(confirmations[i].transactionId.Value);
								if (transaction != null)
								{
					...
					if (notNullOrder != null)
					{
						...
						else if (!mailControl)
						{
							...
							if (getTrans != null)
							{
								...
								{
									var createdUser = db.GetVWSH_UserById(this.createdby.Value);
									var iptalUser = db.GetVWSH_UserById(userId);
									var text = "<h3>Sayın " + createdUser.FullName + ",</h3>";
									text += "<p>" + iptalUser.FullName + " kişisi masraf talebiniz için düzeltme talebinde bulunmuştur.</p>";
									if (!string.IsNullOrEmpty(this.description))
									{
										text += "<p>Açıklaması : " + this.statusDescription + "</p>";
									}

									new Email().Template("Template1", "bos.png", TenantConfig.Tenant.TenantName + " | Masraf Onayı ", text).Send((Int16)EmailSendTypes.MasrafOnay, createdUser.email, "Masraf Düzeltme Talebi", true);
									notification.NotificationSend(createdUser.id, "Masraf Düzeltme Talebi", iptalUser.FullName + " kişisi masraf talebiniz için düzeltme talebinde bulunmuştur.");
								}
							}
						}
					}
