...
{
	public class VMPRD_ProductionTransactionModel : VWPRD_Transaction
	{
		...
		private ResultStatus Update(PRD_Transaction transaction)
		{
			...
			if (this.status == (int)EnumPRD_TransactionStatus.beklemede || DBproduct.Count(a => a.stockType == (int)EnumPRD_ProductStockType.SeriNoluTakip) == 0)
			{
				...
				if (DBResult.result)
				{
					if (this.productionId.HasValue)
					{
						var productionProducts = db.GetPRD_ProductionProductByProductionId(this.productionId.Value);
						...
						if (productionProducts.Count() > 0)
						{
							...
							//DBResult &= db.BulkDeletePRD_ProductionProduct(productionProducts);
						}
					}
				}
			}
		...
		private ResultStatus Insert(Guid? inventoryId)
		{
			...
			for (int i = 0; i < this.items.Count(); i++)
			{
				...
				{
					errorList.AddRange(this.Validator(item));
					if (item.serialCodes != null && item.serialCodes.Where(a => a.ToString() == "").Count() <= 0)
					{
						foreach (var serialcode in (item.serialCodes ?? "").Split(','))
						{
							var winventory = this.inventories.Where(a => a.productId == item.productId && a.serialcode == serialcode).FirstOrDefault();
		...
		private List<string> Validator(VWPRD_TransactionItem item)
		{
			...
			if (serialCodes != null && serialCodes.Where(x => x.Contains("")).Count() <= 0)
			{
			...
			if (this.type == (int)EnumPRD_TransactionType.ZimmetVerme)
			{
				...
				if (user != null && !string.IsNullOrEmpty(user.email))
				{
					...
					if (this.status == (int)EnumPRD_TransactionStatus.islendi)
					{
						...
						notifaction.NotificationSend(user.id,this.createdby, "Personel Zimmet Verme İşlemi", notify, $"/CMP/VWCMP_Storage/Detail?id={this.inputId}");
					}
					...
					if (warehouseSuperVisor.Count() > 0)
					{
						...
						notifaction.NotificationSend(id,this.createdby.Value, "Personel Zimmet Verme İşlemi", notify, $"/CMP/VWCMP_Storage/Detail?id={this.inputId}");
				...
				if (user != null && !string.IsNullOrEmpty(user.email))
				{
					if (this.status == (int)EnumPRD_TransactionStatus.islendi)
					{
					   ...
					   + "aşağıda bulunan envanter(ler) tarafınıza zimmetinizden çıkarılmıştır."
						...
						notifaction.NotificationSend(user.id,this.createdby, "Personel Zimmet Alma İşlemi", notify, $"/CMP/VWCMP_Storage/Detail?id={this.inputId}");
					...
					if (warehouseSuperVisor != null)
					{
						...
						notifaction.NotificationSend(id,this.createdby ,"Personel Zimmet Alma İşlemi", notify, $"/CMP/VWCMP_Storage/Detail?id={this.inputId}");
						new Email().Template("Template1", "personelzimmet.jpg", title, contentSuperVisor)
