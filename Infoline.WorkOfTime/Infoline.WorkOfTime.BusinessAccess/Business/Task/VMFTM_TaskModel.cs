...
namespace Infoline.WorkOfTime.BusinessAccess
{
	public class VMFTM_TaskModel : VWFTM_Task
	{
		...
		public ResultStatus UpdateStaff(Guid? userId, DbTransaction _trans = null)
		{
			...
			{
				var taskUsers = db.GetFTM_TaskUserByTaskId(this.id);
				var taskUserOperations = db.GetFTM_TaskOperationByTaskId(this.id);
				if (taskUserOperations.Any() && taskUserOperations.Where(a => a.status == (int)EnumFTM_TaskOperationStatus.PersonelAtamaYapildi).Count() > 0)
				{
					var deleteOperation = taskUserOperations.Where(a => a.status == (int)EnumFTM_TaskOperationStatus.PersonelAtamaYapildi).ToArray();
					rs &= db.BulkDeleteFTM_TaskOperation(deleteOperation, trans);
				}
				rs &= db.BulkDeleteFTM_TaskUser(taskUsers, trans);
			}
