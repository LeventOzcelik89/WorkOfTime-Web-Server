...
namespace Infoline.WorkOfTime.BusinessAccess
{
	...
	public class VMCMP_TenderModels : VWCMP_Tender
	{
			...
			else
			{
				if (this.Request != null)
				{
					...
					if (dbresult.result)
					{
						...
						if (this.Request != null)
						{
							this.Request.createdby = this.Request.createdby.HasValue ? this.Request.createdby.Value : Guid.NewGuid();
							var user = db.GetSH_UserById(this.Request.createdby.Value);
							var text = "<h3>Sayın " + user.firstname + " " + user.lastname + "</h3>";
							text += "<p>" + this.Request.rowNumber + " kodlu Satın alma talebine teklif girilmiştir.</p>";
							text += "<p>Talep detayını görüntülemek ve işlem yapmak için <a href='" + _siteURL + "/CMP/VWCMP_Request/Detail?id=" + this.Request.id + "'>tıklayınız.</a> </p>";
							text += "<p>Bilgilerinize.</p>";
							new Email().Template("Template1", "satinalma.jpg", _tenantName + " | WorkOfTime | Satış Sipariş Yönetimi", text).Send((Int16)EmailSendTypes.SiparisTeklif, user.email, "Teklif Oluşturuldu", true);
						}
					}
		...
		public SummaryHeadersTender GetMyTenderPageInfo(Guid userId, Guid? presentationId = null)
		{
			...
			{
				return db.GetDBVWCMP_GetMyTenderAccountingSummary(userId, presentationId);

			}
			else
			{
				return db.GetDBVWCMP_GetMyTenderSummary(userId, presentationId);
			}
		}
	}
}
