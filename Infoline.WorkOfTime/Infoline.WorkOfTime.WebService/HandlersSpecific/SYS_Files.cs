...
namespace Infoline.WorkOfTime.WebService.Handler
{
    ...
    public partial class SYS_FilesHandler : BaseSmartHandler
    {
        ...
        [HandleFunction("UploadFile/InsertProfileImage")]
        public void SYS_FilesUpdate(HttpContext context)
        {
            ...
            var rsFiles = new ResultStatus() { result = false };
            var isHaveProfile = db.GetSysFilesByDataTableAndFileGroupAndDataId(sysfiles.DataTable, sysfiles.FileGroup, new Guid(sysfiles.DataId.ToString()), null);

            if (isHaveProfile == null)
            {
                rsFiles = db.InsertSYS_Files(sysfiles);
            }
            else
            {
                rsFiles = db.UpdateSYS_Files(sysfiles, true);
            }
