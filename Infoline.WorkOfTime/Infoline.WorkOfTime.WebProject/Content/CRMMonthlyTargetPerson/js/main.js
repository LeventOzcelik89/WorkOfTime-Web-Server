var _targetPerson = {
    ...
    Data: {
        ...
        GetAllData: function () {
            ...
            $.each(_targetPerson.Data.TableData, function (di, ditem) {
        ...
        PostData: function () {
            ...
            $('#MonthlyTargets').val(JSON.stringify(_targetPerson.Data.GetAllData()));
    ...
    Table: {
        RemoveLastColumn: function (data) {
            ...
            var _containerTable = _targetPerson.container.find('[data-type="' + data.Type + '"] [data-type="TableContainer"]');
        ...
        AddNewColumn: function (data) {
            ...
            $.each(_containerTable.find('[data-type="numeric"]:not(.k-input)'), function (i, item) {
                ...
                _targetPerson.fn_MakeNumeric($(item));
        ...
        MakeTable: function (data) {
            ...
            var res = '<table class="table table-bordered table-custom">' +
                _targetPerson.Table.MakeHeaders(data) +
                '<tbody>';

            $.each(data.Products, function (pi, pitem) {

                res += _targetPerson.Table.GetTableLine(data, pitem);

            });

            //  newLine
            res += _targetPerson.Table.GetTableLine(data, null);
            res += '</tbody></table>';
            ...
            $.each(_containerTable.find('[data-type="dropdown"]'), function (i, item) {
                ...
                _targetPerson.fn_MakeDropdown($(item));
                $(item).trigger('change');
            ...
            $.each(_containerTable.find('[data-type="IsFocus"], [data-type="IsMultipleFocus"]'), function (i, item) {
                ...
                _targetPerson.fn_MakeCheckBox($(item));
        ...
        UpdateHeaders: function (data) {
            ...
            var _containerTable = _targetPerson.container.find('[data-type="' + data.Type + '"] [data-type="TableContainer"]');
            var newHeaders = _targetPerson.Table.MakeHeaders(data).replace('<thead>', '').replace('</thead>', '');
        ...
        GetTableLine: function (data, product) {
            ...
            res += '<td><select data-type="dropdown" value="' + (product != undefined ? product.id : '') + '"</select></td>';
    ...
    fn_MakeCheckBox: function (item) {
        ...
        $(item).on('change', function () {
            ...
            var data = _targetPerson.Data.TableData[$(this).parents('fieldset').eq(0).attr('data-type')];
            ...
            if (proc == undefined) {
                ...
                //  newLine
                $(this).parents('tbody').eq(0).append(_targetPerson.Table.GetTableLine(data, null))
    ...
    fn_MakeDropdown: function (item) {
        ...
        $(item)
            ...
            .kendoMultiSelect({
                ...
                change: function () {
                    var data = _targetPerson.Data.TableData[this.element.parents('fieldset').eq(0).attr('data-type')];
                    ...
                    if (proc == undefined) {
                        ...
                        //  newLine
                        this.element.parents('tbody').eq(0).append(_targetPerson.Table.GetTableLine(data, null));
    ...
    TargetGroupTypes: [
        ...
        //  { type: 1, Name: 'Takım Lideri Performans Hedef Tablosu' },
        //  { type: 2, Name: 'Satış Sorumlusu Performans Hedef Tablosu' }
    ],
    ...
    init: function () {
        ...
        _targetPerson.initOthers();

        GetJsonDataFromUrl('/CRM/VWCRM_MonthlyTargetPerson/GetTargetsByGroupId', { groupId: Model.GroupId }, function (res) {

            _targetPerson.TargetDatas = $.Enumerable.From(res)
    ...
    initOthers: function () {
        ...
        if (_targetPerson.TargetDatas == null) {
            return;
        }

        var _enumTargetDatas = $.Enumerable.From(_targetPerson.TargetDatas);

        _targetPerson.container.html(null);

        $.each(_targetPerson.TargetGroupTypes, function (i, type) {

            _targetPerson.container.append(
            ...
            var _containerTable = _targetPerson.container.find('[data-type="' + type.type + '"] [data-type="TableContainer"]');
            ...
            _targetPerson.Data.TableData[type.type] = {
                Type: type.type,
                Products: products
            };

            _targetPerson.Table.MakeTable(_targetPerson.Data.TableData[type.type]);
