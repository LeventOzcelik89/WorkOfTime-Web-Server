...
(function (factory) {
    ...
    OrgChart.prototype = {
        ...
        init: function (opts) {
            ...
            var $chart = this.$chart = $j('<div>', {
                'data': { 'options': this.options },
                'class': 'orgchart' + (this.options.chartClass !== '' ? ' ' + this.options.chartClass : '') + (this.options.direction !== 't2b' ? ' ' + this.options.direction : ''),
                'click': function (event) {
                    if (!$j(event.target).closest('.node').length) {
        ...
        panStartHandler: function (e) {
            var $chart = $j(e.delegateTarget);
            if ($j(e.target).closest('.node').length || (e.touches && e.touches.length > 1)) {
        ...
        bindPan: function () {
            ...
            $j(document).on('mouseup touchend', { 'chart': this.$chart }, this.panEndHandler);
        },
        //
        unbindPan: function () {
            this.$chartContainer.css('overflow', 'auto');
            this.$chart.off('mousedown touchstart', this.panStartHandler);
            $j(document).off('mouseup touchend', this.panEndHandler);
        },
        //
        ...
        bindZoom: function () {
            ...
            $j(document).on('touchmove', { 'oc': this }, this.zoomingHandler);
            $j(document).on('touchend', { 'oc': this }, this.zoomEndHandler);
        },
        unbindZoom: function () {
            this.$chartContainer.off('wheel', this.zoomWheelHandler);
            this.$chartContainer.off('touchstart', this.zoomStartHandler);
            $j(document).off('touchmove', this.zoomingHandler);
            $j(document).off('touchend', this.zoomEndHandler);
        },
        //
        ...
        buildJsonDS: function ($li) {
            ...
            $li.children('ul').children().each(function () {
                ...
                subObj.children.push(that.buildJsonDS($j(this)));
            });
            return subObj;
        },
        //
        ...
        loopChart: function ($chart) {
            ...
            $tr.siblings(':last').children().each(function () {
                if (!subObj.children) { subObj.children = []; }
                subObj.children.push(that.loopChart($j(this)));
            });
            return subObj;
        },
        //
        ...
        hideParentEnd: function (event) {
            $j(event.target).removeClass('sliding');
        ...
        isVisibleNode: function (index, elem) {
            return this.getNodeState($j(elem)).visible;
        },
        //
        ...
        showSiblings: function ($node, direction) {
            ...
            if (!this.getNodeState($node, 'parent').visible) {
                ...
                this.repaint(parent);
                $j(parent).addClass('sliding').removeClass('slide-down').one('transitionend', this.showRelatedParentEnd);
            }
        ...
        startLoading: function ($edge) {
            ...
            $j('.oc-export-btn' + (this.options.chartClass !== '' ? '.' + this.options.chartClass : '')).prop('disabled', true);
            return true;
        },
        ...
        endLoading: function ($edge) {
            ...
            $j('.oc-export-btn' + (this.options.chartClass !== '' ? '.' + this.options.chartClass : '')).prop('disabled', false);
        },
        ...
        nodeEnterLeaveHandler: function (event) {
            var $node = $j(event.delegateTarget), flag = false;
        ...
        nodeClickHandler: function (event) {
            ...
            $j(event.delegateTarget).addClass('focused');
        },
        ...
        topEdgeClickHandler: function (event) {
            ...
            var that = this;
            var $topEdge = $j(event.target);
            var $node = $j(event.delegateTarget);
        ...
        createGhostNode: function (event) {
            ...
            if (!document.querySelector('.ghost-node')) {
                ...
                nodeCover = $j(ghostNode).children().get(0);
            }
              ...
              .find('.node').each(function (index, node) {
                  if ($dragHier.index(node) === -1) {
                      if (opts.dropCriteria) {
                          if (opts.dropCriteria($dragged, $dragZone, $j(node))) {
                              $j(node).addClass('allowedDrop');
                          }
                      } else {
                          $j(node).addClass('allowedDrop');
                      }
        ...
        dragstartHandler: function (event) {
            ...
            this.filterAllowedDropNodes($j(event.target));
        },
        //
        dragoverHandler: function (event) {
            event.preventDefault();
            if (!$j(event.delegateTarget).is('.allowedDrop')) {
        ...
        dropHandler: function (event) {
            var $dropZone = $j(event.delegateTarget);
        ...
        createNode: function (nodeData, level, opts) {
            ...
            var $nodeDiv = $j('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
        ...
        buildHierarchy: function ($appendTo, nodeData, level, opts, callback) {
            ...
            if (Object.keys(nodeData).length > 1) { // if nodeData has nested structure
                $nodeWrapper = isVerticalNode ? $appendTo : $j('<table>');
                if (!isVerticalNode) {
                ...
                .done(function ($nodeDiv) {
                    if (isVerticalNode) {
                        ...
                        $nodeWrapper.append($j('<tr/>').append($j('<td' + (hasChildren ? ' colspan="' + $childNodes.length * 2 + '"' : '') + '></td>').append($nodeDiv)));
                    }
                    if (callback) {
                        callback();
                    }
            ...
            if (hasChildren) {
                ...
                if (isVerticalLayer) {
                    ...
                    $nodeLayer = $j('<tr class="nodes' + isHidden + '">');
                    $nodeWrapper.append(lineLayer).append($nodeLayer);
                }
                // recurse through children nodes
                $.each($childNodes, function () {
                    var $nodeCell = isVerticalLayer ? $j('<li>') : $j('<td colspan="2">');
        ...
        buildParentNode: function ($currentRoot, nodeData, opts, callback) {
            ...
            $.when(this.createNode(nodeData, 0, opts || $currentRoot.closest('.orgchart').data('options')))
              .done(function ($nodeDiv) {
                  $table.append($j('<tr class="hidden">').append($j('<td colspan="2">').append($nodeDiv.removeClass('slide-up').addClass('slide-down'))));
                ...
                this.buildHierarchy($nodeChart.closest('.orgchart'), nodeData, 0, opts, function () {
                    if (++nodeCount === siblingCount) {
                        that.complementLine($nodeChart.next().children('tr:last')
                          .children().eq(insertPostion).after($j('<td colspan="2">')
        ...
        export: function (exportFilename, exportFileextension) {
            ...
            if ($j(this).children('.spinner').length) {
                return false;
            }
            var $chartContainer = this.$chartContainer;
            ...
            html2canvas(sourceChart, {
                ...
                'onclone': function (cloneDoc) {
                    $j(cloneDoc).find('.canvasContainer').css('overflow', 'visible')
