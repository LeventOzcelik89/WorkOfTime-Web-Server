...
function FavoriteMenu(userId) {
    GetJsonDataFromUrl("/General/GetAllMyFavorites", { userId: userId }, function (resp) {
        ...
        var dataSikKullanilanlar = $.Enumerable.From(resp.Object.resultFavorites).OrderByDescending(c => c.Value.Count).ToArray();
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (typeof (res)) {
                            ...
                            if (_this.hasClass("unitName")) {
                                _this.text(kendo.toString(parseInt(res), "N0") + " Adet");
                            }
                        }
                    }
                });
