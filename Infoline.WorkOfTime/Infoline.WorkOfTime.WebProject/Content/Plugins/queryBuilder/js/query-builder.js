...
(function (root, factory) {
    ...
    var Selectors = QueryBuilder.selectors = {
        ...
        times_container: '.rule-times-container',
        timesInput_container: '.rule-timesInput-container',
        timesOperator_container: '.rule-timesOperator-container',
        timesToleranceInput_container: '.rule-timesToleranceInput-container',

        rule_times: '.rule-times-container [name$=_times]',
        rule_timesOperator: '.rule-timesOperator-container [name$=_timesOperator]',
        rule_timesInput: '.rule-timesInput-container [name$=_timesInput]',
        rule_timesToleranceInput: '.rule-timesToleranceInput-container [name$=_timesToleranceInput]'
    ...
    QueryBuilder.DEFAULTS = {
        ...
        inputs_separator: '',
        select_placeholder: '------',
        select_timeplaceholder: '------',
        tolerance_placeholder: '------',
        timeInput_placeholder: '',
        ...
        templates: {
            ...
            operatorSelect: null,
            timesSelect: null,
            timesOperatorSelect: null,
        },
        display_empty_time: true,
        default_time: null,
        times: [{ id: 'AN', label: 'Anlık' }, { id: 'SR', label: 'Süresince' }, { id: 'AVG', label: 'Ortalama' }],
        timesOperator: [{ id: 'HOUR', label: 'Saat' }, { id: 'DAY', label: 'Gün' }, { id: 'MONTH', label: 'Ay' }],
    ...
    QueryBuilder.prototype.init = function ($el, options) {
        ...
        this.times = this.settings.times;
        this.timesOperator = this.settings.timesOperator;
        ...
        this.times = this.checkTimes(this.times);
    ...
    QueryBuilder.prototype.checkTimes = function (times) {
        var definedTimes = [];

        if (!times || times.length === 0) {
            Utils.error('Config', 'Missing times list');
        }

        times.forEach(function (time, i) {
            if (!time.id) {
                Utils.error('Config', 'Missing time {0} id', i);
            }
            if (definedTimes.indexOf(time.id) != -1) {
                Utils.error('Config', 'Time "{0}" already defined', time.id);
            }
            definedTimes.push(time.id);
            ...
            if (!time.optgroup) {
                ...
                if (!this.settings.optgroups[time.optgroup]) {
                    this.settings.optgroups[time.optgroup] = time.optgroup;
                }
            }

            if (time.placeholder) {
                if (time.placeholder_value === undefined) {
                    time.placeholder_value = -1;
                }
                Utils.iterateOptions(time.values, function (key) {
                    if (key == time.placeholder_value) {
                        Utils.error('Config', 'Placeholder of time "{0}" overlaps with one of its values', time.id);
                    }
                });
            }
        }, this);
        ...
        if (this.status.has_optgroup) {
            times = Utils.groupSort(times, 'optgroup');
        }
    ...
    QueryBuilder.prototype.bindEvents = function () {
        ...
        this.$el.on('change.queryBuilder', Selectors.rule_times, function () {

            var $rule = $(this).closest(Selectors.rule_container);
            Model($rule).times = $(this).val() != '-1' ? $(this).val() : null;

            //  $rule.find(Selectors.filter_container).empty();
            //  $rule.find(Selectors.operator_container).empty();
            //  $rule.find(Selectors.value_container).empty();
            //  $rule.find(Selectors.timesOperator_container).empty();
            //  $rule.find(Selectors.timesInput_container).empty();

            //  Model($rule).filter = null;
            //  Model($rule).operator = null;
            //  Model($rule).timesOperator = 'HOUR';
            //  Model($rule).timesInput = null;
            //  Model($rule).value = null;

            if (Model($rule).times == 'SR') {
                if (Model($rule).$el.find(Selectors.timesToleranceInput_container).length == 0) {
                    self.createRuleTimesOperator(Model($rule));
                }
                //  Model($rule).$el.find(Selectors.filter_container).empty();
            } else if (Model($rule).times == 'AVG') {
                if (Model($rule).$el.find(Selectors.timesOperator_container).length == 0) {
                    self.createRuleTimesOperator(Model($rule));
                }
                //  Model($rule).$el.find(Selectors.filter_container).empty();
            } else {
                if (Model($rule).$el.find(Selectors.filter_container).length == 0) {
                    self.createRuleFilters(Model($rule));
                }
            }
        ...
        // rule timesOperator change
        this.$el.on('change.queryBuilder', Selectors.rule_timesOperator, function () {

            var $rule = $(this).closest(Selectors.rule_container);
            Model($rule).timesOperator = $(this).val() != '' ? $(this).val() : null;

        });

        // rule timesInput change
        this.$el.on('change.queryBuilder', Selectors.rule_timesInput, function () {

            var $rule = $(this).closest(Selectors.rule_container);
            Model($rule).timesInput = $(this).val() != '' ? $(this).val() : null;

        });

        this.$el.on('change.queryBuilder', Selectors.rule_timesToleranceInput, function () {

            var $rule = $(this).closest(Selectors.rule_container);
            Model($rule).timesToleranceInput = $(this).val() != '' ? $(this).val() : null;
            ...
            'update': function (e, node, field, value, oldValue) {
                if (node instanceof Rule) {
                    switch (field) {
                        ...
                        case 'times':
                            self.updateRuleTimes(node, oldValue);
                            break;

                        case 'timesOperator':
                            self.updateRuleTimesOperator(node, oldValue);
                            break;

                        case 'timesInput':
                            self.updateRuleTimesInput(node, oldValue);
                            break;

                        case 'timesToleranceInput':
                            self.updateRuleTimesToleranceInput(node, oldValue);
                            break;
    ...
    QueryBuilder.prototype.createRuleTimes = function (rule) {
        var times = this.change('createRuleTimes', this.times, rule);
        var $timeSelect = $(this.getRuleTimesSelect(rule, times));

        rule.$el.find(Selectors.times_container).html($timeSelect);
        rule.__.times = times[0];
        rule.$el.find(Selectors.rule_times + '[value="' + times[0].id + '"]').trigger('change');

        this.trigger('afterCreateRuleTimes', rule);
    };

    QueryBuilder.prototype.createRuleTimesOperator = function (rule) {

        var $timesOperatorContainer = rule.$el.find(Selectors.timesOperator_container).empty();

        if (rule.times == null || (rule.times != 'SR' && rule.times != 'AVG'))
            return;

        var operators = this.getTimesOperators(rule.filter);
        var $operatorSelect = $(this.getRuleTimeOperatorSelect(rule, operators));

        $timesOperatorContainer.html($operatorSelect);
        rule.__.timesOperator = operators[0];
        rule.$el.find(Selectors.rule_timesOperator + '[value="' + operators[0].type + '"]').trigger('change')

        this.trigger('aftercreateRuleTimesOperator', rule, operators);

    };

    QueryBuilder.prototype.createRuleTimesInput = function (rule) {

        if (rule.times == null || rule.timesOperator == null)
            return;

        var $timeInputElem = $(this.getRuleTimesInput(rule));

        rule.$el.find(Selectors.timesInput_container).html($timeInputElem);

        this.trigger('aftercreateRuleTimesInput', rule);
    };

    QueryBuilder.prototype.createRuleTimesToleranceInput = function (rule) {

        if (rule.times == null || rule.timesOperator == null || rule.timesInput == null)
            return;

        var $toleranceElem = $(this.getRuleTimesToleranceInput(rule));

        rule.$el.find(Selectors.timesToleranceInput_container).html($toleranceElem);

        this.trigger('aftercreateRuleTimesToleranceInput', rule);
    };
    ...
    QueryBuilder.prototype.updateRuleTimes = function (rule, previousValue) {

        rule.$el.find(Selectors.rule_times + '[value="' + rule.times + '"]').prop('checked', true);

        if (Model(rule).times != null) {

            if (Model(rule).times == 'SR') {

                if (rule.$el.find(Selectors.rule_timesOperator).length == 0) {
                    this.createRuleTimesOperator(Model(rule));
                }

                if (rule.$el.find(Selectors.rule_timesToleranceInput).length == 0) {
                    this.createRuleTimesToleranceInput(Model(rule));
                }
                ...
                rule.$el.find(Selectors.timesToleranceInput_container).empty();
                //  rule.$el.find(Selectors.operator_container).empty();
                //  rule.$el.find(Selectors.value_container).empty();

            }
            else {

                Model(rule).timesOperator = null;
                rule.$el.find(Selectors.timesOperator_container).empty();

                this.createRuleFilters(Model(rule));
            }
    ...
    QueryBuilder.prototype.updateRuleTimesOperator = function (rule, previousValue) {

        rule.$el.find(Selectors.rule_timesOperator + '[value="' + rule.timesOperator + '"]').prop('checked', true);

        if (Model(rule).timesOperator != null) {
            if (rule.$el.find(Selectors.rule_timesInput).length == 0) {
                this.createRuleTimesInput(Model(rule));
            }
        } else {
            Model(rule).timesInput = null;
            rule.$el.find(Selectors.timesInput_container).empty();
        }

    };

    QueryBuilder.prototype.updateRuleTimesInput = function (rule, previousValue) {

        rule.$el.find(Selectors.rule_timesInput).val(rule.timesInput);

        if (Model(rule).timesInput != null) {
            if (rule.$el.find(Selectors.rule_timesToleranceInput).length == 0) {

                if (Model(rule).__.times == 'SR') {
                    this.createRuleTimesToleranceInput(Model(rule));
                } else if (Model(rule).__.times == 'AVG') {
                    this.createRuleFilters(Model(rule));
                }

            }
        } else {
            Model(rule).timesToleranceInput = null;
            rule.$el.find(Selectors.timesToleranceInput_container).empty();
        }

    };

    QueryBuilder.prototype.updateRuleTimesToleranceInput = function (rule, previousValue) {

        rule.$el.find(Selectors.rule_timesToleranceInput).val(rule.timesToleranceInput);

        if (Model(rule).timesToleranceInput != null) {
            if (rule.$el.find(Selectors.rule_filter).length == 0) {
                this.createRuleFilters(Model(rule));
            }
        } else {
            if (Model(rule).times == 'SR' || Model(rule).times == 'AVG') {
                Model(rule).filter = null;
                //  rule.$el.find(Selectors.filter_container).empty();
            }
        }
    ...
    QueryBuilder.prototype.validate = function () {
        ...
        var valid = (function parse(group) {
            ...
            group.each(function (rule) {
                ...
                if (!rule.times) {
                    self.triggerValidationError(rule, 'no_times', null);
                    errors++;
                    return;
                }

                if ((rule.times == 'SR' || rule.times == 'AVG') && !rule.timesOperator) {
                    self.triggerValidationError(rule, 'no_timesOperator', null);
                    errors++;
                    return;
                }
                if ((rule.times == 'SR' || rule.times == 'AVG') && rule.timesOperator != null && !rule.timesInput) {
                    self.triggerValidationError(rule, 'no_timesInput', null);
                    errors++;
                    return;
                }
                if ((rule.times == 'SR') && rule.timesOperator != null && !rule.timesToleranceInput) {
                    self.triggerValidationError(rule, 'no_tolerance', null);
                    errors++;
                    return;
                }
        ...
        var out = (function parse(group) {
            ...
            group.each(function (rule) {
                ...
                var ruleData = {
                    ...
                    times: rule.times ? rule.times : null,
                    timesOperator: rule.timesOperator ? rule.timesOperator : null,
                    timesInput: rule.timesInput ? rule.timesInput : null,
                    timesToleranceInput: rule.timesToleranceInput ? rule.timesToleranceInput : null,
            ...
            data.rules.forEach(function (item) {
                ...
                if (item.rules !== undefined) {
                    ...
                    if (!item.empty) {
                        ...
                        model.timesOperator = item.timesOperator;
                        model.timesInput = item.timesInput;
                        model.timesToleranceInput = item.timesToleranceInput;
...
<li id="{{= it.rule_id }}" class="rule-container"> \
  ...
  {{? it.settings.display_errors }} \
    <div class="error-container"><i class="{{= it.icons.error }}"></i></div> \
  {{?}} \
  <div class="rule-times-container"></div> \
  <div class="rule-timesOperator-container"></div> \
  <div class="rule-timesInput-container"></div> \
  <div class="rule-timesToleranceInput-container"></div> \
...
<select class="form-control {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
    ...
    //    QueryBuilder.templates.timesSelect = '\
    //{{ var optgroup = null; }} \
    //<select class="form-control" name="{{= it.rule.id }}_times"> \
    //  {{? it.settings.display_empty_time }} \
    //    <option value="-1">{{= it.settings.select_timeplaceholder }}</option> \
    //  {{?}} \
    //  {{~ it.times: time }} \
    //    {{? optgroup !== time.optgroup }} \
    //      {{? optgroup !== null }}</optgroup>{{?}} \
    //      {{? (optgroup = time.optgroup) !== null }} \
    //        <optgroup label="{{= it.translate(it.settings.optgroups[optgroup]) }}"> \
    //      {{?}} \
    //    {{?}} \
    //    <option value="{{= time.id }}">{{= it.translate(time.label) }}</option> \
    //  {{~}} \
    //  {{? optgroup !== null }}</optgroup>{{?}} \
    //</select>';

    QueryBuilder.templates.timesSelect = '\
    <div class="radio builderRadioGrup clearfix"> \
        {{~ it.times: time }} \
            <input id="{{= it.rule.id + time.id }}" type="radio" value="{{= time.id }}" name="{{= it.rule.id }}_times" /> \
            <label for="{{= it.rule.id + time.id }}">{{= time.label }}</label> \
        {{~}} \
	</div>';

    QueryBuilder.templates.timesOperatorSelect = '\
    <div class="radio builderRadioGrup clearfix"> \
        {{~ it.operators: operator }} \
            <input id="{{= it.rule.id + operator.type }}" type="radio" value="{{= operator.type }}" name="{{= it.rule.id }}_timesOperator" /> \
            <label for="{{= it.rule.id + operator.type }}">{{= it.lang.operators[operator.label] || operator.label }}</label> \
        {{~}} \
	</div>';
    ...
    //{{?}} \
    //{{ var optgroup = null; }} \
    //<select class="form-control {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_timesOperator"> \
    //  {{~ it.operators: operator }} \
    //    {{? optgroup !== operator.optgroup }} \
    //      {{? optgroup !== null }}</optgroup>{{?}} \
    //      {{? (optgroup = operator.optgroup) !== null }} \
    ...
    //      {{?}} \
    //    {{?}} \
    //    <option value="{{= operator.type }}">{{= it.lang.operators[operator.label] || operator.label }}</option> \
    //  {{~}} \
    //  {{? optgroup !== null }}</optgroup>{{?}} \
    //</select>';
    ...
    QueryBuilder.prototype.getTimesOperators = function (filter) {

        var result = [];
        result.push({ apply_to: ['number'], multiple: false, nb_inputs: 1, optgroup: null, type: 'HOUR', label: 'Saat' });
        result.push({ apply_to: ['number'], multiple: false, nb_inputs: 1, optgroup: null, type: 'DAY', label: 'Gün' });
        result.push({ apply_to: ['number'], multiple: false, nb_inputs: 1, optgroup: null, type: 'MONTH', label: 'Ay' });

        return this.change('afterGetTimesOperators', result, filter);
    };

    QueryBuilder.prototype.getRuleTimeOperatorSelect = function (rule, operators) {

        var h = this.templates.timesOperatorSelect({
            builder: this,
            rule: rule,
            operators: operators,
            icons: this.icons,
        ...
        return this.change('getRuleTimesOperatorSelect', h, rule);
    };


    QueryBuilder.prototype.getRuleTimesSelect = function (rule, times) {
        var h = this.templates.timesSelect({
            builder: this,
            rule: rule,
            times: times,
            icons: this.icons,
        ...
        return this.change('getRuleTimesSelect', h, rule);
    };

    QueryBuilder.prototype.getRuleTimesToleranceInput = function (rule, value_id) {

        var name = rule.id + '_timesToleranceInput';
        return this.change('afterGetRuleTimesToleranceInput', '<input class="form-control" placeholder="' + this.settings.tolerance_placeholder + '" type="number" min="0" max="100" name="' + name + '" />', rule, name);

    };


    QueryBuilder.prototype.getRuleTimesInput = function (rule, value_id) {

        var name = rule.id + '_timesInput';
        return this.change('afterGetRuleTimesInput', '<input class="form-control" placeholder="' + this.settings.timeInput_placeholder + '" type="number" min="0" name="' + name + '" />', rule, name);
    ...
    var Rule = function (parent, $el) {
        ...
        this.__.times = null;
        this.__.timesOperator = null;
        this.__.timesInput = null;
        this.__.timesToleranceInput = null;
    };

    Rule.prototype = Object.create(Node.prototype);
    Rule.prototype.constructor = Rule;

    Model.defineModelProperties(Rule, ['times', 'timesOperator', 'timesInput', 'timesToleranceInput', 'filter', 'operator', 'value']);
...
<div' + style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  <label for="' + id + '">' + val + '</label> \
            ...
            var sql = (function parse(group) {
                ...
                group.rules.forEach(function (rule) {
                    if (rule.rules && rule.rules.length > 0) {
                        ...
                        //  builder.settings.QCT_Options.SP_QCT_ProcInstant
                        //  builder.settings.QCT_Options.SP_QCT_ProcThroughout
                        //  builder.settings.QCT_Options.SP_QCT_CheckAverage

                        //  <add key="QCT_ProcThroughout" value="_#_[dbo].[SP_CheckThroughout] '{TableName}', '{stationId}', '{DateDate}', '{Period}', {PeriodValue}, '{Tolerance}', {Query}_#_" />
                        //  <add key="QCT_ProcInstant" value="_#_[dbo].[SP_CheckInstant] '{TableName}', {stationId}, '{DateDate}', '{Query}'_#_" />
                        //  <add key="QCT_ProcAverage" value="_#_[dbo].[SP_QCT_CheckAverage] '{TableName}', {StationId}, {DateDate}, '{Period}', {PeriodValue}, {Parameter}, {Query}_#_" />

                        if (rule.times == 'SR') {

                            ruleExpression = self.settings.QCT_Options.QCT_ProcThroughout
                                .replace('{Period}', rule.timesOperator)
                                .replace('{PeriodValue}', rule.timesInput)
                                .replace('{Query}', ruleExpression)
                                .replace('{Tolerance}', rule.timesToleranceInput);

                        } else if (rule.times == 'AVG') {

                            ruleExpression = self.settings.QCT_Options.QCT_ProcAverage
                                .replace('{Period}', rule.timesOperator)
                                .replace('{PeriodValue}', rule.timesInput)
                                .replace('{Parameter}', rule.id)
                                .replace('{Tolerance}', rule.timesToleranceInput)
                                .replace('{Query}', ruleExpression);

                        } else {

                            ruleExpression = self.settings.QCT_Options.QCT_ProcInstant
                                .replace('{Query}', ruleExpression);
                        }
    ...
    QueryBuilder.regional['en'] = {
        ...
        "errors": {
            "no_times": "No time selected",
            "no_timesOperator": "No time operator selected",
            "no_timesInput": "No time input value",
            "no_tolerance": "No Tolerance Value",
