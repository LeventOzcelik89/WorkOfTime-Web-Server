...
namespace Infoline.WorkOfTime.WebProject.Areas.CMP.Controllers
{
    public class VWCMP_CompanyController : Controller
    {
        ...
        public ContentResult DataSource([DataSourceRequest] DataSourceRequest request)
        {
            ...
            condition = UpdateDataSource(condition, userStatus);
        ...
        [PageInfo("Firma&Cari Listesi Dropdown Verileri", SHRoles.Personel, SHRoles.HakEdisBayiPersoneli)]
        ...
        public static SimpleQuery UpdateDataSource(SimpleQuery query, PageSecurity userStatus)
        {
            if (userStatus.AuthorizedRoles.Contains(new Guid(SHRoles.HakEdisBayiPersoneli)))
            {
                if (userStatus.user.CompanyId == null)
                {
                    query.Filter = new BEXP { Operand1 = (COL)"id", Operator = BinaryOperator.NotEqual, Operand2 = (VAL)null };
                    return query;
                }
