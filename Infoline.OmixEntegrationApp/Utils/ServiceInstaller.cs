    ...
    public class SvcInstaller
    {
        ...
        public static void Uninstall(string ServiceName)
        {
            ...
            try
            {
                ...
                {
                    throw new ApplicationException("Windows Servis Yüklenemedi...");
                }
                try
                {
                    StopService(service);
                    ...
                    if (ret == 0)
                    {
                        ...
                        throw new ApplicationException("Windows Servis Silinemedi : " + error);
                    }
                }
                finally
                {
        ...
        string FileName, bool start)
        {
            ...
            try
            {
                ...
                if (service == IntPtr.Zero)
                {
                    throw new ApplicationException("Windows Servis Yüklenirken Hata Aldı");
                }
                if (start)
                {
                    try
        ...
        public static void StartService(string Name)
        {
            ...
            try
            {
                ...
                {
                    throw new ApplicationException("Servis Başlatılamadı...");
                }
                try
                {
        ...
        private static ServiceState GetServiceStatus(IntPtr hService)
        {
            ...
            {
                throw new ApplicationException("Servis Sorgusunda Hata Alındı...");
            }
        ...
        private static IntPtr OpenSCManager(ServiceManagerRights Rights)
        {
            ...
            {
                throw new ApplicationException("Kontrol Yönetimine Bağlanılamadı...");
            }
            return scman;
        }
        [DllImport("ole32.dll")]
        ...
        public static void ServisInstall(String ServiceName, bool ReInstall, bool Start, string path)
        {
            if (ReInstall)
            {
                ServisUnInstall(ServiceName);
                SvcInstaller.Install(ServiceName, ServiceName, path, true);
            }
            else
            {
                var s = SvcInstaller.GetServiceStatus(ServiceName);
                if (s == ServiceState.NotFound)
                    SvcInstaller.Install(ServiceName, ServiceName, path, true);
            }
            if (Start)
                ServisStart(ServiceName, false, "");
        }

        public static void ServisUnInstall(String ServiceName)
        {
            var s = SvcInstaller.GetServiceStatus(ServiceName);
            if (s != ServiceState.NotFound)
            {
                if (s == ServiceState.Run)
                    SvcInstaller.StopService(ServiceName);
                SvcInstaller.Uninstall(ServiceName);
            }
        }

        public static void ServisStart(String ServiceName, bool Install, string path)
        {
            if (Install)
                ServisInstall(ServiceName, false, true, path);
            else
            {
                var s = SvcInstaller.GetServiceStatus(ServiceName);
                // if (s != ServiceState.NotFound)
                {
                    if (s == ServiceState.Stop)
                        SvcInstaller.StartService(ServiceName);
                }
            }
        }

        public static void ServisStop(String ServiceName)
        {
            var s = SvcInstaller.GetServiceStatus(ServiceName);
            // if (s != ServiceState.NotFound)
            {
                if (s == ServiceState.Starting || s == ServiceState.Run)
                    SvcInstaller.StopService(ServiceName);
            }
        }

        static void ServisReStart(String ServiceName, string path)
        {
            ServisStop(ServiceName);
            ServisStart(ServiceName, true, path);
        }
    }
