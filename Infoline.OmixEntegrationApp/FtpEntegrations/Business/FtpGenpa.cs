        ...
        public PRD_EntegrationFiles[] GetFilesInFtp(DateTime processDate)
        {
            ...
            try
            {
                ...
                using (XmlReader reader = XmlReader.Create(content))
                {
                    ...
                    for (int i = 0; i < tagName.Count; i++)
                    {
                        ...
                        {
                            directoryItems.Add(new FtpEntegration.Entities.DirectoryItem { Name = fileName, DateCreated = date });
                        }
                    }
                }
            }
            ...
            foreach (var file in directoryItems)
            {
                ...
                entegrationFileList.Add(new PRD_EntegrationFiles
                {
                    ...
                    CreateDateInFtp = file.DateCreated,
                    DistributorName = DistributorName,
                    DistributorId = DistributorId,
                    FileName = file.Name,
                    FileNameDate = FtpEntegration.Utils.Tools.GetDateFromFileNameForGenpa(file.Name, "yyyyMMdd"),
