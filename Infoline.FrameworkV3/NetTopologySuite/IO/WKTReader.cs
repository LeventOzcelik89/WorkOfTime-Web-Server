...
namespace NetTopologySuite.IO
{
    ...
    public class WKTReader : ITextGeometryReader
    {
		...
		/// the stream.
		/// </returns>
        private Coordinate[] GetCoordinates(IEnumerator<Token> tokens, Boolean skipExtraParenthesis, ref Boolean hasZ, ref Boolean hasM)
		{
            string nextToken = GetNextEmptyOrOpener(tokens);
            if (nextToken.Equals("EMPTY")) 
                return new Coordinate[]{};
            var coordinates = new List<Coordinate>();
            coordinates.Add(GetPreciseCoordinate(tokens, skipExtraParenthesis, ref hasZ, ref hasM));
        ...
        /// <returns></returns>
        private Coordinate GetPreciseCoordinate(IEnumerator<Token> tokens, Boolean skipExtraParenthesis, ref Boolean hasZ, ref Boolean hasM)
        {
            ...
            if (IsNumberNext(tokens))
            {
                ...
                if (!Double.IsNaN(coord.M)) hasM = true;
            }
        ...
        private IPoint ReadPointText(IEnumerator<Token> tokens, IGeometryFactory factory) 
        {
            ...
            var hasZ = false;
            var hasM = false;
            var coord = GetPreciseCoordinate(tokens, false, ref hasZ, ref hasM);
            var point = factory.CreatePoint(ToSequence(hasZ, hasM, coord));
            /*var closer = */GetNextCloser(tokens);
            return point;
        }

        private ICoordinateSequence ToSequence(bool hasZ, bool hasM, params Coordinate[] coords)
        {
            var dimensions = hasM ? 4 : (hasZ ? 3 : 2);
            ...
            if (dimensions == 4)
            {
                ...
                {
                    seq.SetOrdinate(i, Ordinate.Z, coords[i].Z);
                    seq.SetOrdinate(i, Ordinate.M, coords[i].M);
                }
            }
            return seq;
        }
        ...
        private ILineString ReadLineStringText(IEnumerator<Token> tokens, IGeometryFactory factory)
        {
            var hasZ = false;
            var hasM = false;
            var coords = GetCoordinates(tokens, false, ref hasZ, ref hasM);
            return factory.CreateLineString(ToSequence(hasZ, hasM, coords));
        }
        ...
        private ILinearRing ReadLinearRingText(IEnumerator<Token> tokens, IGeometryFactory factory)
        {
            ...
            var hasM = false;
            var coords = GetCoordinates(tokens, false, ref hasZ, ref hasM);
            return factory.CreateLinearRing(ToSequence(hasZ, hasM, coords));
        }
        ...
        private IMultiPoint ReadMultiPointText(IEnumerator<Token> tokens, IGeometryFactory factory) 
        {
            var hasZ = false;
            var hasM = false;
            var coords = GetCoordinates(tokens, true, ref hasZ, ref hasM);
            return factory.CreateMultiPoint(ToPoints(ToSequence(hasZ, hasM, coords), factory));
        }
