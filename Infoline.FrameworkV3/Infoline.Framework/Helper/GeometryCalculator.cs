...
namespace Infoline.Framework.Helper
{
    public class GeometryCalculator
    {
        ...
        private Point GetPointFromGeographyPoint(SqlGeography item)
        {
            return new Point { X = item.Lat.Value, Y = item.Long.Value };
        }
        ...
        public Point ComputeOffset(Point p, double distance, double angle, double radius = 6371000)
        {
            ...
            return new Point { X = ToDegrees(npx), Y = ToDegrees(npy) };
        }
        ...
        public double ComputeKMH(Point p1, Point p2, double Minute)
        {
        ...
        public Point ComputeMiddlePoint(Point p1, Point p2)
        {
            ...
            return new Point { X = ToDegrees(a3), Y = ToDegrees(t3) };
        ...
        public List<Point> GetBoundingBox(SqlGeography item)
        {
            ...
            res.Add(new Point { X = list.Min(a => a.point.X), Y = list.Max(a => a.point.Y) });
            res.Add(new Point { X = list.Max(a => a.point.X), Y = list.Max(a => a.point.Y) });
            res.Add(new Point { X = list.Max(a => a.point.X), Y = list.Min(a => a.point.Y) });
            res.Add(new Point { X = list.Min(a => a.point.X), Y = list.Min(a => a.point.Y) });
